...
Data is made available to Tableau as JSON files. Metrics Accelerator will create the JSON files in the directory specified by the  `tableau.outputDir` (poller) or by `loaderDetails.outputDir` (API). Files will be written according to the following rules:
- Platform Metrics (rawData = false): **platform_metrics_agg_yyyyMMddHHmmssSSS.json**
- Platform Metrics (rawData = true): **platform_metrics_raw_yyyyMMddHHmmssSSS.json**
- Platform Benefits: _not implemented_

**WARNING:** If Metrics Accelerator is running on Cloudhub or Runtime Fabric, Tableau Strategy can't be used because it's not possible to access the file system where the files will be generated. In order to generate the files when Metrics Accelerator is running on Cloudhub or Runtime Fabric, follow these steps:
...
1. Before opening a workbook in Tableau Desktop, make sure that you have at least one JSON file available (either generated by Tableau Strategy or manually saved using JSON Strategy).
...
Tableau will use all JSON files available in the directory where the selected file is stored as data sources.

Follow official Tableau documentation: https://www.tableau.com/support/help
